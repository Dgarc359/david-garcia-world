...
export class BackendStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: BackendStackProps) {
    ...
    const subdomainName = "about";

    const prodNonprodDeletionPolicy =
      stage === "prod" ? cdk.RemovalPolicy.RETAIN : cdk.RemovalPolicy.DESTROY;

    const staticSiteBucket = new cdk.aws_s3.Bucket(
      this,
      `${props?.stackName}-static-site-bucket`,
      {
        ...
        autoDeleteObjects: stage === "prod" ? true : false,
      },
    );

    const staticSiteDeploymentBucket =
      new cdk.aws_s3_deployment.BucketDeployment(
        this,
        `${props?.stackName}-static-site-deployment-bucket`,
        {
    ...
    const ARecord = new cdk.aws_route53.ARecord(
      ...
      `${props?.stackName}-site-alias-record`,
      {
        ...
        target: cdk.aws_route53.RecordTarget.fromAlias(
          new cdk.aws_route53_targets.BucketWebsiteTarget(staticSiteBucket),
        ),
